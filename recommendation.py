...
def calculate_item_item_similarity(ratings):
    ...
    ii_similarity = coo_matrix(
        (ratings['rating'].values.astype(np.float16), (ratings['movieId'], ratings['userId']))
        ).tocsr()
    ...
    for _, normalize in tqdm(cases, desc='CF', position=0):
        ...
        for r in test.itertuples():
            ...
            if d == 0:
                ...
                p = r.normalization_delta + np.sum(tmp) / d
                s += np.float64((p - r.rating) ** 2)
        s /= (num_samples - no_match)
        s = np.sqrt(s)
        err_ii.append({ 'normalize': normalize, 'err': s, 'no_match': no_match })
...
if __name__ == "__main__":
    ...
    test_knn(ratings, args.size, args.min_ratings)